...
def resnext(units, num_stage, filter_list, num_classes, data_type, num_group=32,
    ...
    label = mx.sym.one_hot(indices=label, depth=num_classes + 1)
    label = mx.sym.sum(label, axis=1)
    label = mx.sym.slice_axis(label, axis=1, begin=0, end=num_classes, name='multi_hot_label')
    ...
    cls = mx.symbol.LogisticRegressionOutput(data=fc1, label=label, name='multi_sigmoid')
    return cls
